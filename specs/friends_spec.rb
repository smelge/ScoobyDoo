...
class TestFriends < MiniTest::Test
  ...
  def test_likes_to_eat
    result = check_food(@person4,"ratatouille")
    assert_equal(true, result)
  end

  def test_hates_to_eat
    result = check_food(@person4,"jam")
  ...
  def test_add_friend
    result = new_friend(@person1, "Ducky")
    assert_equal(5, result)
  end

    # 5. For a given person, remove a specific name from their list of friends
    # (hint: Same as above, testing for the length of the array should be sufficient)
  def test_kill_friend
    result = doomed(@person3, "Velma")
  ...
  def test_moneybags
    result = lending(@person5,@person1,25)
    assert_equal([75,26],result)
  end
  ...
  def test_lonely_people
    result = so_sad(@people)
    assert_equal(["Daphne"],result)
  end
end
